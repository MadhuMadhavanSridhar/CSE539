...
vector<vector<unsigned char>> AESState::insert128Hex(string hexInput){
...
string AESState::validateStringCut(string hexString){
	string cutString = "";
	try {
...
void AESState::validateVect(vector<vector<unsigned char>> vectInput){ // THIS IS VALIDATING IT's 4x4 OR LESS
	int count = 0;
	int count2 = 0;
...
string AESState::hexOutput(vector<vector<unsigned char>> vectInput){
	string output = "";
	string tempString = "";
	stringstream ss;
...
void AESState::checkValidState(){ //Validate for each step, this is checking that if it's not a full 32 hex chars it's at least padded.
	try{
		if (currentState.size()>4){
		    throw 4;
		} 
        for(int i =0; i<currentState.size()-1; i++){
            if(currentState[i].size()>4){
                throw 5;
            }
        }
...
unsigned char AESState::addition(unsigned char p1, unsigned char p2) {
    return p1 ^ p2;
}

unsigned char AESState::multiply(unsigned char p1, unsigned char p2){
    std::bitset<16> temp; 
...
vector<unsigned char> AESState::addCoef(vector<unsigned char> coef1, vector<unsigned char> coef2){
...
vector<unsigned char> AESState::modPro(vector<unsigned char> coef1, vector<unsigned char> coef2){
...
void AESState::MixColumns(){ //Adapted
	checkValidState();
...
void AESState::invMixColumns(){ //Adapted
	checkValidState();
...
vector<unsigned char> AESState::subWord(vector<unsigned char> in)
{
...
vector<unsigned char> AESState::rotWord(vector<unsigned char> in)
{
    char temp;
    temp = in[0];
...
vector<vector<unsigned char>> AESState::KeyExpansion(vector<vector<unsigned char>> cipherkey){
    ...
    while (i < Nb * (Nr + 1)){ 
        ...
        if(i % Nk == 0){
            ...
            temp = addCoef(subWord(rotWord(temp)), Rcon);
...
void AESState::AddRoundKey(int rround, vector<vector<unsigned char>> KeyExpansion){
    int l = rround * Nb;
    for(int i = 0; i < Nb; i++){
...
void AESState::invAddRoundKey(int rround, vector<vector<unsigned char>> KeyExpansion){
...
//public
AESState::AESState(){ //really shouldn't use for much
...
AESState::AESState(vector<vector<unsigned char>> input){
	validateVect(input);
	currentStateString = hexOutput(input);
	currentState = input;
}

AESState::AESState(vector<vector<unsigned char>> input, int inputNk){
	validateVect(input);
	currentStateString = hexOutput(input);
	currentState = input;
	try{
...
vector<vector<unsigned char>> AESState::getCurrentStateVect(){
	return currentState;
}

void AESState::setCurrentStateVect(vector<vector<unsigned char>> inputVect){
	validateVect(inputVect);
	currentState = inputVect;
	currentStateString = hexOutput(currentState);
}

string AESState::getCurrentStateString(){
	return currentStateString;
}

void AESState::setCurrentStateString(string hexInput){
	currentStateString = validateStringCut(hexInput);
	currentState = insert128Hex(currentStateString);

}

void AESState::viewCurrentState(){
	 //ONLY SUPPORTS 4x4 AT THE MOMENT, should ONLY need that
	//checkValidState(); // must be padded, must not throw exception.
    vector<unsigned char> columnVector1 = currentState[0];
    vector<unsigned char> columnVector2 = currentState[1];
    vector<unsigned char> columnVector3 = currentState[2];
    vector<unsigned char> columnVector4 = currentState[3];
...
void AESState::Cipher(vector<vector<unsigned char>> cipherKey){
...
void AESState::invCipher(vector<vector<unsigned char>> cipherKey){
